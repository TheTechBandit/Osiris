...
namespace Osiris
{
    public class ArtemisBarrageExiled : BasicMove
    {
        public override string Name { get; } = "Artemis' Barrage";
        public override string Owner { get; } = "Odysseus";
        public override string Description { get; } = "Barrage the enemy team with arrows. Deal 3d6 damage to all enemies and flip a coin. if the coin is heads, this ability does not go on cooldown.";
        ...
        public ArtemisBarrageExiled() : base()
        {
            
        }

        public ArtemisBarrageExiled(bool newmove) : base(newmove)
        {
        ...
        {
            List<int> rolls = RandomGen.RollDice(3, 6);
            await MessageHandler.DiceThrow(inst.Location, "3d6", rolls);
